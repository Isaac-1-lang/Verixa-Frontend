...
import { AlertTriangle, Archive, BookOpen, LoaderCircle, Pencil, Plus, RefreshCw, Search, X } from 'lucide-react';
...
export default function TrainingSectionPage() {
  ...
  const renderField = (field) => {
    ...
    if (field.type === 'relation') return <select {...common}><option value="">Select…</option>{relationRows[field.source].map(row => <option key={row.id} value={row.id}>{relationLabel(row)}</option>)}</select>;
  ...
  return <div className="space-y-5">
    <header className="flex flex-col justify-between gap-4 rounded-2xl border border-navy/8 bg-white p-6 sm:flex-row sm:items-center"><div><h1 className="text-2xl font-bold text-navy">{config.title}</h1><p className="mt-1 text-sm text-navy/45">{config.description}</p></div><button onClick={openCreate} className="inline-flex items-center justify-center gap-2 rounded-lg bg-navy px-5 py-3 text-sm font-bold text-white"><Plus size={16}/>Add {config.singular}</button></header>
